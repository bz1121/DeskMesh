...
| 锁屏输入或 Ctrl+Alt+Del 被无意开放 | 锁屏扩展默认关闭；仅当前已认证控制会话、同会话 `LogonUI.exe` 和明确设置共同放行；安全注意序列只能由 LocalSystem 服务模拟已认证管道客户端调用官方 `SendSAS`；不修改本地策略、不采集 Winlogon 画面或凭据 |
| AI 服务泄露隐私或返回恶意指令 | 默认关闭；本机脱敏；API Key 使用独立 DPAPI 存储；非本机端点强制 HTTPS；响应限制大小/时长；仅允许无自由参数的固定修复动作，执行前由本机状态复核 |
...
- 开机登录、注销后的登录、BIOS/UEFI 及内核/反作弊隔离边界；UAC 仅在用户显式安装可选安全桌面组件后支持固定键鼠转发，且仍须人工决定是否同意；锁屏扩展只支持已登录后再锁定的会话，且不能传输 Winlogon 画面；
...
- DeskMesh 应用管理员不是 Windows 管理员；登录不会提升权限。可选 UAC 组件是独立的 Windows 管理员安装动作，不会自动同意 UAC。锁屏扩展默认关闭，仅扩展到当前已登录后再锁定的会话；不支持开机或注销后的登录。
...
涉及 AI 诊断的改动必须覆盖：旧设置默认关闭、API 地址协议限制、脱敏、密钥密文存储、响应大小/JSON 深度、未知或重复动作拒绝、建议过期，以及自动修复的本机状态门控。任何新增动作都必须是无自由参数的显式代码分支并单独审查，不能提供通用命令、脚本、路径、注册表或插件执行入口。
