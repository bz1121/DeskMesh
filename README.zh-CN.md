...
- 可选的 UAC 安全桌面组件：由本机管理员显式安装，仅转发已认证的固定键鼠事件，不关闭 UAC，也不会自动点击“是”。
...
DeskMesh 是软件转发，不会真的把 USB 设备重新插到另一台电脑。普通 `SendInput` 无法跨越 UAC 安全桌面；可在每台目标电脑的“管理员设置”中显式安装可选安全桌面组件。该组件只转发 DeskMesh 的固定键盘/鼠标协议，Windows 仍会显示 UAC 提示并要求用户明确确认，它不能执行命令或自动同意提权。锁屏、登录前、`Ctrl+Alt+Del`、BIOS 和部分反作弊游戏仍不支持，这些场景请使用硬件 KVM。
